import IMainController, { EnumDogState, TImageState } from './interfaces/IMainController'
import fileImageShadowDog from '../../assets/images/shadow_dog.png'

export default class AnimationController extends IMainController {
  ...
  animate() {
    ...
    this.ctxContext?.drawImage(
      ...
      tempThing.loc[this.frameX].x,
      tempThing.loc[this.frameX].y,
      this.spriteWidth,
      this.spriteHight,
      0,
      0,
  ...
  addSelectAnimation(): void {
    try {
      const tempSelectBox: HTMLSelectElement | null = document.createElement(`select`)
      ...
      for (const key in EnumDogState as any) {
        if (Object.prototype.hasOwnProperty.call(EnumDogState as any, key)) {
          const opt: HTMLOptionElement | null = document.createElement(`option`)
          opt.text = key
